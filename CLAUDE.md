...
```
3. Reference the relevant milestone from `wiki/dev/roadmap.md`
...
Note: when operating under the wiki task lifecycle, DEVLOG entries are generated by the building/fixing agent at the moment a task moves into `wiki/tasks/done/`, and link back to the task file.

## Knowledge Base & Task Workflow — START HERE

Masonry has an LLM-maintained wiki at `wiki/` that serves as the single source of truth for design, architecture, decisions, and project management. **Before doing any non-trivial work, read:**

1. `wiki/SCHEMA.md` — the rules for how the wiki is structured, workflows for ingest/query/lint, and the task lifecycle.
2. `wiki/INDEX.md` — catalog of every wiki page; your entry point for finding information.
3. `wiki/LOG.md` — recent activity (ingests, queries, task transitions).

The wiki has two tracks:
- `wiki/game/` — player/community-facing: systems, creatures, lore, UI.
- `wiki/dev/` — implementation-facing: architecture, subsystems, decisions, roadmap, known-issues.

Project management is file-based kanban at `wiki/tasks/` (`inbox/` → `scoped/` → `in-progress/` → `done/`, with `blocked/` as a side branch). Status = folder. Transitions = `git mv`. Agent prompts for the lifecycle live at `wiki/tasks/PROMPT_*.md`.

When the user asks you to investigate, plan, or implement something substantial, default to: (1) check `wiki/INDEX.md` for existing knowledge, (2) check `wiki/tasks/` for existing tickets, (3) use the appropriate agent prompt if a full lifecycle is warranted.
